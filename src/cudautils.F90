module cudautils
   ...
   interface 
     ...
     real(c_float) function cu_Ctrace(hcublas, d_A, nrow) &
     !integer(c_int) function cu_Ctrace(d_A, nrow, d_trace) &
     ...
     real(c_double) function cu_Ztrace(hcublas, d_A, nrow) &
...
!~-~-~-~-~-~-~-~-~-~-~-~ DATA MOVEMENT ROUTINES  ~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-
   ...
   subroutine copyFromGPU_sp(A)
     type(c_DNS), intent(inout), target :: A     
     integer :: err
   ...
   subroutine copyFromGPU_dp(A)
     type(z_DNS), intent(inout), target :: A     
     integer :: err
...
!~-~-~-~-~-~-~-~-~-~-~-~ MATRIX COMPUTATION ROUTINES  ~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-
   ...
   subroutine matmul_gpu_sp(hcublas, alpha, A, B, beta, C, dagger)
     ...
     istat = cu_CmultMat(hcublas, A%nrow, C%ncol, B%nrow, alpha, A%d_addr, &
             & B%d_addr, beta, C%d_addr, 0)
     endif
     if (dagger == 'dag_1st') then
     istat = cu_CmultMat(hcublas, A%ncol, B%ncol, A%nrow, alpha, A%d_addr, &
             & B%d_addr, beta, C%d_addr, 1)
     endif
     if (dagger == 'dag_2nd') then
     istat = cu_CmultMat(hcublas, A%nrow, B%nrow, A%ncol, alpha, A%d_addr, &
   ...
   subroutine matsum_gpu_sp(hcublas, alpha, A, beta, B, C, dagger)
     ...
     if (.not.present(dagger)) then  
       istat = cu_Cmatsum(hcublas, A%nrow, B%ncol, alpha, A%d_addr, beta, B%d_addr, C%d_addr, 0)
     endif
     if (dagger == 'dag_1st') then
       istat = cu_Cmatsum(hcublas, A%ncol, B%ncol, alpha, A%d_addr, beta, B%d_addr, C%d_addr, 1)
     endif  
     if (dagger == 'dag_2nd') then
       istat = cu_Cmatsum(hcublas, A%nrow, B%nrow, alpha, A%d_addr, beta, B%d_addr, C%d_addr, 2)
     endif  
   ...
   subroutine trace_gpu_sp(hcublas, A, trace)
      type(cublasHandle), intent(in) :: hcublas     
      type(c_DNS), intent(inout) :: A
      ...
      trace = cu_Ctrace(hcublas, A%d_addr, A%nrow)
   ...
   subroutine matmul_gpu_dp(hcublas, alpha, A, B, beta, C, dagger)
     ...
     if (.not.present(dagger)) then
     istat = cu_ZmultMat(hcublas, A%nrow, C%ncol, B%nrow, alpha, A%d_addr, &
             & B%d_addr, beta, C%d_addr, 0)
     endif
     if (dagger == 'dag_1st') then
     istat = cu_ZmultMat(hcublas, A%ncol, B%ncol, A%nrow, alpha, A%d_addr, &
             & B%d_addr, beta, C%d_addr, 1)
     endif
     if (dagger == 'dag_2nd') then
     istat = cu_ZmultMat(hcublas, A%nrow, B%nrow, A%ncol, alpha, A%d_addr, &
             & B%d_addr, beta, C%d_addr, 2)
     endif
   ...
   subroutine matsum_gpu_dp(hcublas, alpha, A, beta, B, C, dagger)
     ...
     if (.not.present(dagger)) then  
       istat = cu_Zmatsum(hcublas, A%nrow, B%ncol, alpha, A%d_addr, beta, B%d_addr, C%d_addr, 0)
     endif
     if (dagger == 'dag_1st') then
       istat = cu_Zmatsum(hcublas, A%ncol, B%ncol, alpha, A%d_addr, beta, B%d_addr, C%d_addr, 1)
     endif  
     if (dagger == 'dag_2nd') then
       istat = cu_Zmatsum(hcublas, A%nrow, B%nrow, alpha, A%d_addr, beta, B%d_addr, C%d_addr, 2)
     endif  
   ...
   subroutine trace_gpu_dp(hcublas, A, trace)
      type(cublasHandle), intent(in) :: hcublas     
      type(z_DNS), intent(inout) :: A
      ...
      trace = cu_Ztrace(hcublas, A%d_addr, A%nrow)
