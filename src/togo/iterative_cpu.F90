...
use inversions
  !Blocks of gsmr,gsml,Gr are allocated inside the subroutines, unlike their GPU version --> DONE: GPU version updated

  !Problem: cannot define the same interface for gpu and cpu: gpu has both sp and dp implementations.
  !Maybe the quickest solution to inlcude cpu and gpu subroutines is to define an if for the compiler inside iterative.F90 --> DONE

  !Destruction of matrices has to be modified to include GPU deallocation --> DONE

  !ESH has to be copyied to GPU --> DONE
  
  !Problem with init_blkmat and Gn --> DONE 

  !Problem with Gr_columns --> DONE
...
contains
  ...
  module subroutine calculate_gsmr_blocks(negf,ESH,sbl,ebl,gsmr,keepall)
  ...
  module subroutine calculate_gsml_blocks(negf,ESH,sbl,ebl,gsml)
  ...
  module subroutine calculate_Gr_tridiag_blocks(negf,ESH,gsml,gsmr,Gr,sbl,ebl)
  ...
  module subroutine calculate_Gn_tridiag_blocks(negf,ESH,SelfEneR,frm,ref,struct,gsml,gsmr,Gr,Gn)
