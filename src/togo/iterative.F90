...
#:endif
    ...
    call calculate_gsmr_blocks(negf,ESH,nbl,rbl+1,gsmr)
    call calculate_gsml_blocks(negf,ESH,1,lbl-1,gsml)
    ...
    call calculate_Gr_tridiag_blocks(negf,ESH,gsml,gsmr,Gr,rbl)
    call calculate_Gr_tridiag_blocks(negf,ESH,gsml,gsmr,Gr,rbl+1,nbl)
...
#:endif
    ...
    if ( ncont.gt.1 ) then
      call calculate_gsml_blocks(negf,ESH,1,nbl-2,gsml)
    endif
    ...
    if (ncont == 2 .and. size(ni) == 1 .and. nt == 1) then
      ...
      do icpl = 1, size(ni)
        ...
        if (icpl == 1) then
          ...
          call calculate_Gr_tridiag_blocks(negf,ESH,gsml,gsmr,Gr,1)
          if (nt.gt.1) then
            call calculate_Gr_tridiag_blocks(negf,ESH,gsml,gsmr,Gr,2,nt)
          end if
        else
          ! When more contacts are present sometimes we can re-use previous GF
          ! if nt1 > nt extend the Gr calculation
          if (nt1 .gt. nt) then
            call calculate_Gr_tridiag_blocks(negf,ESH,gsml,gsmr,Gr,nt+1,nt1)
            nt = nt1
          endif
        end if
