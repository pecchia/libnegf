...
#:else
  interface 
     module subroutine calculate_gsmr_blocks(negf,ESH,sbl,ebl,keepall)
       type(TNegf), intent(in) :: negf
       type(z_DNS), dimension(:,:), intent(in) :: ESH
       integer, intent(in) :: sbl,ebl                 
       logical, intent(in), optional :: keepall
     end subroutine
     module subroutine calculate_gsml_blocks(negf,ESH,sbl,ebl)
       type(Tnegf), intent(in) :: negf
       type(z_DNS), dimension(:,:), intent(in) :: ESH
       integer, intent(in) :: sbl,ebl                       
     end subroutine
     module  subroutine calculate_Gr_tridiag_blocks(negf,ESH,sbl,ebl)
       type(Tnegf), intent(in) :: negf
       type(z_DNS), dimension(:,:), intent(in) :: ESH
       integer, intent(in) :: sbl
       integer, intent(in), optional :: ebl
     end subroutine
     module subroutine calculate_Gn_tridiag_blocks(negf,ESH,SelfEneR,frm,ref,struct,Gn)
       type(Tnegf), intent(in) :: negf
       type(z_DNS), dimension(:,:), intent(in) :: ESH
       type(z_DNS), dimension(:,:), intent(inout) :: Gn
       type(z_DNS), dimension(:), intent(in) :: SelfEneR
...
CONTAINS
  ...
  subroutine calculate_Gr(negf,E,SelfEneR,Tlc,Tcl,gsurfR,Grout,outer)
    ...
    call calculate_gsmr_blocks(negf,ESH,nbl,2)

    call allocate_blk_dns(Gr,nbl)

    call calculate_Gr_tridiag_blocks(negf,ESH,1)
    call calculate_Gr_tridiag_blocks(negf,ESH,2,nbl)
  ...
  subroutine calculate_Gn_neq_components(negf,E,SelfEneR,Tlc,Tcl,gsurfR,frm,Glout,outblocks)
    ...
    call calculate_gsmr_blocks(negf,ESH,nbl,rbl+1)
    call calculate_gsml_blocks(negf,ESH,1,lbl-1)
    ...
    call calculate_Gr_tridiag_blocks(negf,ESH,rbl)
    call calculate_Gr_tridiag_blocks(negf,ESH,rbl+1,nbl)
    call calculate_Gr_tridiag_blocks(negf,ESH,rbl-1,1)
...
#:endif
  ...
  subroutine iterative_meir_wingreen(negf,E,SelfEneR,Tlc,Tcl,gsurfR,frm,curr_mat)
    ...
    if ( ncont.gt.1 ) then
      call calculate_gsml_blocks(negf,ESH,1,nbl-2)
    endif
...
#:endif
  ...
  subroutine calculate_transmissions(negf,H,S,Ec,SelfEneR,ni,nf,str,tun_proj,tun_mat)
    ...
    if (ncont == 2 .and. size(ni) == 1 .and. nt == 1) then
      call allocate_gsm(gsmr,nbl)
      call calculate_gsmr_blocks(negf,ESH,nbl,2,.false.)
      call allocate_blk_dns(Gr,nbl)
      call calculate_Gr_tridiag_blocks(negf,ESH,1)
